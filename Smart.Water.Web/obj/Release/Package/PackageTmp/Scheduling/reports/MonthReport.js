...
/*打开默认当前的时间*/
function date() {
    $("#txtTime").val(moment().format('YYYY-MM'));
}
            ...
            switch (StationKey.substring(6, 12)) {
                case "030199":
                    if (v.TagKey() != "030199_006" && v.TagKey() != "030199_002" && v.TagKey() != "030199_003") {
                        ReportMonth.CurrentTags.push(v);
                    }
                    break;
                case "030202":
...
function CreatTable(dataSet) {
    ...
    var dayCount = new Date(timestr.split("-")[0], timestr.split("-")[1], 0).getDate();
    ReportMonth.CurrentColumns = [];
    var datasourse = [];
    var columns = [];
    var lineitem = [];
    ...
    colNameHtml += "<div class='table_title'><ul  class='columnUl'><li>日期</li>";
    ReportMonth.CurrentColumns.push("时间");
    $.each(ReportMonth.CurrentTags, function (i, v) {
        ...
        ReportMonth.CurrentColumns.push(v.TagName() + "最大值");
        ReportMonth.CurrentColumns.push(v.TagName() + "最大值时间");
        ReportMonth.CurrentColumns.push(v.TagName() + "最小值");
        ReportMonth.CurrentColumns.push(v.TagName() + "最小值时间");
        ReportMonth.CurrentColumns.push(v.TagName() + "平均值");
    });
    itemHtml += "</ul></div>";
    colNameHtml += "</ul></div>";
    ...
    for (var i = 1; i <= dayCount; i++) {
        ...
        if (databytime.length == 0) {
            for (var j = 0; j < ReportMonth.CurrentTags.length; j++) {
                lineitem.push("");
                lineitem.push("");
                lineitem.push("");
                lineitem.push("");
    ...
    ReportMonth.CurrentDataSet = datasourse;
    var dataHtml = "";
    ...
    var tableWidth = ReportMonth.CurrentTags.length * 450 + 90;
...
function CreatAlertTable(dataSet) {
    ReportMonth.CurrentAlertColmuns = [];
    ReportMonth.CurrentAlertDataSet = [];
    var columns = [];
    var lineitem = [];
    var datasource = [];
    queryData = [];
    ...
    $.each(columns, function (i, v) {
        ReportMonth.CurrentAlertColmuns.push(v.title);
    });
    if (dataSet.length > 0) {
            ...
            switch (type) {
                case "030199":
                    if (vv.TagKey() != "030199_006" && vv.TagKey() != "030199_002" && vv.TagKey() != "030199_003") {
                        ReportMonth.CurrentTags.push(vv);
                    }
                    break;
                case "030202":
...
function query() {
    ...
    NProgress.start();
    $.getJSON(url + "/iot/report/general/month/" + StationKey + "/" + time + "?callback=?", function (data) {
        NProgress.done();
        if (data.status == 500) {
            alert(data.data);
            return;
    ...
    $.getJSON(url + "/iot/reportalert/general/month/" + StationKey + "/" + time + "?callback=?", function (data) {
        if (data.status == 500) {
            alert(data.data);
            return;
        }
...
function Export() {
    ...
    $("#hidData").val(JSON.stringify(ReportMonth.CurrentDataSet));

    $("#hidColumn").val(JSON.stringify(ReportMonth.CurrentColumns));

    $("#hidAlertData").val(JSON.stringify(queryData));

    $("#hidAlertColumn").val(JSON.stringify(ReportMonth.CurrentAlertColmuns));
    //GeneralMonthReport
    $("#form1").attr("action", "GeneralReport.ashx?Action=GeneralMonthReport&time=" + $("#txtTime").val() + "&name=" + encodeURI($("#StationList :selected").text()) + "&pd=" + pd);
    $("#form1").submit();
}