    ...
    $("#btnQuery").click(function () {
        ...
        switch (value) {
            ...
            case "month":
                ...
                NProgress.start();
                $.each(parent.Robin.Data.monitor, function (i, v) {
                    $.getJSON(url + "/iot/monthreport/" + v.BMID + "/" + time + "?callback=?", function (data) {
                        if (data.status == 500) {
                            noty({ text: data.data, type: "warning", layout: "topCenter", timeout: 5000 });
                            NProgress.done();
                            return;
            ...
            case "year":
                ...
                NProgress.start();
                $.each(parent.Robin.Data.monitor, function (i, v) {
                    $.getJSON(url + "/iot/yearreport/" + v.BMID + "/" + time + "?callback=?", function (data) {
                        if (data.status == 500) {
                            noty({ text: data.data, type: "warning", layout: "topCenter", timeout: 5000 });
                            NProgress.done();
                            return;
...
function CreatDayTable(dataSet) {
    ...
    $.each(Report.CurrentTags, function (i, v) {
        ...
        colNameHtml += "<li class='left-border'>最大值(" + v.Units() + ")</li><li>时间</li><li>最小值(" + v.Units() + ")</li><li>时间</li><li>平均值(" + v.Units() + ")</li><li>总值(m³)</li>";
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "最大值(" + v.Units() + ")");
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "时间");
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "最小值(" + v.Units() + ")");
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "时间");
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "平均值(" + v.Units() + ")");
        Report.CurrentColumns.push(v.StationName() + v.TagName() + "总值(" + v.Units() + ")");
    });
    itemHtml += "</ul></div>";
    colNameHtml += "</ul></div>";
    /*构造table*/
