...
$(function () {
    ...
    var date = new Date();
    $('#txtMonthTime').val(moment(date).format('YYYY-MM'));
    //获取树
    $.ajax({
        type: 'POST',
        dataType: 'json',
...
function btnQuery() {
    ...
    switch (value) {
        ...
        case "monthAndmonth":
            ...
            //根据现在的时间推算要查询的历史时间
            var oldTime;
            var s = parseInt(time.substring(5, 6));//月份前一位
            var ss = parseInt(time.substring(6, 7));//月份后一位
            var sss = parseInt(time.substring(0, 4));//年
            if (s == 0) {
                if (ss == 1) {
                    oldTime = sss - 1 + "-12"
                } else {
                    oldTime = time.substring(0, 6) + (ss - 1);
                }
            } else {
                if (ss == 0) {
                    oldTime = sss + "-09";
            ...
            $.getJSON(url + "/iot/report/general/month/" + jcdID + "/" + time + "?callback=?", function (data) {
                ...
                $.getJSON(url + "/iot/report/general/month/" + jcdID + "/" + oldTime + "?callback=?", function (data) {
                    if (data.status == 500) {
                        noty({ text: data.data, type: "warning", layout: "topCenter", timeout: 5000 });
                        return;
                    }
                    resultOld = resultOld.concat(data.data);
                    //传入查询的数据初始化table和chart                      
                    initTableChart(resultNow, resultOld, 'month');
                });
            });
            //释放导出按钮点击功能
        ...
        case "yearAndyear":
            ...
            //根据现在的时间推算要查询的历史时间
            var oldtime = parseInt(time) - 1 + "-01"
            NProgress.start();
            time = time + "-01";
            var resultNow = [];
            var resultOld = [];
            $.getJSON(url + "/iot/report/general/year/" + jcdID + "/" + time + "?callback=?", function (data) {
                ...
                resultNow = resultNow.concat(data.data);
                $.getJSON(url + "/iot/report/general/year/" + jcdID + "/" + oldtime + "?callback=?", function (data) {
                    if (data.status == 500) {
                        noty({ text: data.data, type: "warning", layout: "topCenter", timeout: 5000 });
                        return;
                    }
                    resultOld = resultOld.concat(data.data);
                    //传入查询的数据初始化table和chart                      
                    initTableChart(resultNow, resultOld, 'year');
                });
            });
            $("#btnExport").removeAttr('disabled').removeAttr('title');
            break;
...
//初始化表格和图表
function initTableChart(resultNow, resultOld, type) {
    Report.queryData = [];
    Report.jCDAllData = [];
    //表格数据历史
    Report.queryDataOld = [];
    switch (type) {
        case 'month':
            ...
            var title = jcdName + moment(month).format('YYYY年MM月') + "日同比分析";
            ...
            var chartdataOld = [];
            var changedata = [];            
            $.each(Report.queryData, function (i, v) {
                $.each(Report.queryDataOld, function (ii, vv) {
                    if (v.SJ == vv.SJ) {
                        v.LSZZ = vv.ZZ;
                        if (parseFloat(v.LSZZ) == 0) {
                            v.BHL = "0.00";
                        } else {
                            v.BHL = ((parseFloat(v.ZZ) - parseFloat(v.LSZZ)) / parseFloat(v.LSZZ) * 100).toFixed(2);
                        }
                        chartdata.push([v.SJ+"", v.ZZ]);
                        chartdataOld.push([vv.SJ+"",vv.ZZ]);
                        changedata.push([v.SJ+"", v.BHL]);
                    }
                })
            })          
            initTable(Report.queryData);
            initChart(title, chartdata, changedata,chartdataOld, type, units);
            break;
        case 'year':
            var title = jcdName +  moment($("#txtYearTime").val()).format('YYYY年') + " 月同比分析"
            //*构造queryData
            for (var i = 1; i <= 12; i++) {
                //某一点的所有检测项数据               
                Report.jCDAllData = [];
                ...
                { "mData": "PJZ", "sTitle": "平均值(m³/h)", "sWidth": "10%" },
                 {
                  ...
                  {
                      "mData": "LSZZ", "sTitle": "历史总值(m³)", "sWidth": "10%"
                  },
                   {
                       "mData": "BHL", "sTitle": "变化率(%)", "sWidth": "15%"
                   },
        ]
    });
...
* @param {string} chartdataOld柱状图数据
* @param {string} changedata 曲线图数据
* @param {string} type 类型根据不同的类型计算
* @param {string} units 单位
*/
var initChart = function (title, chartdata, changedata, chartdataOld,type, units) {
    ...
    option = {
        ...
        tooltip: {
            ...
            formatter: function (params) {              
                ...
                var valueOld = "去年同期值：" + params[1].data[1] + units;
                var change = "变化率" + params[2].data[1] + "%";
                var marker = params[0].marker;
                return marker + name + "\<br>" + marker + value + "\<br>" + marker + valueOld + "\<br>" + marker + change;
            },
        },
        xAxis: {
            axisPointer: {
...
function Export() {
    ...
    switch (type) {
        case "yearAndyear":
            ...
            title = jcdName + time + "按月环比分析";
            break;
        case "monthAndmonth":
            time = $("#txtMonthTime").val();
            title = jcdName + time + "按日环比分析";
            break;     
    }
    var pd = isIE();
    $("#form1").attr("action", "Handler.ashx?Action=ExportResultData&pd=" + pd + "&title=" + title + "&type=" + "环比");
    $("#form1").submit();
}
